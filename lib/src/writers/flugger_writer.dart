...
class FluggerWriter {
  ...
  FluggerWriter({required this.options});
  ...
  Future<void> write(List<FluggerGeneratorResult> results) async {
    await writeToFile(
      '${options.extensions_output_path}/map_extensions.dart',
      mapExtensions,
    );
    await writeToFile(
      '${options.extensions_output_path}/list_extensions.dart',
      listExtensions,
    );
  }
  ...
  Future<void> finalWrite() async {
    await writeBarrelFilesAndExports(options.models_output_path);
    await writeBarrelFilesAndExports(options.extensions_output_path);
  }
  ...
  Future<void> writeBarrelFilesAndExports(String path) async {
    ...
    final files = entities.whereType<File>().where(
      (file) => !file.path.endsWith('_all.dart'),
    );
